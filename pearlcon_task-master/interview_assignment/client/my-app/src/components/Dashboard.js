	...
	<>
		...
		<Col sm={6}>
			...
			{posts.posts.map((post,index) => {
					...
					var counting = '';	
					var likeButton = ''
					likeButton=<a className="Likes" onClick={()=>setLikes(post._id)} >Like</a>	  
					
				if(likes.likes != null && likes.likes.length > 0)
				{
						let result = likes.likes.map((value,index) => {
							
							 if(value ? value.post_id === post._id :'' )
						  {
				...
				{*/
					...
					//var test = likes.likes.map(value => value)
					//console.log('test==',test)
					//console.log('test.length==',test.length)
						//console.log('likes.likes==',likes.likes.map(value => Object.keys(value).length))
...
//	console.log('likeButton==',likeButton)
				...
				var Test = ''
					...
					likeButton=<button className="Likes" onClick={()=>setLikes(post._id)} >Like</button>	  
				
				if(likes.likes != null && likes.likes.length > 0)
				{
						//console.log(likes.likes.map(value => value))
						let result =likes.likes.map((value,index) => {
						  if( value.post_id === post._id)
						  {
								likeButton=<button className="Likes" onClick={()=>setLikes(post._id)} >LIKED</button>	  
						  }
						  else
						  {
							likeButton=<button className="Likes" onClick={()=>setLikes(post._id)} >Like</button>
...
button_display=<button className="Likes" onClick={()=>setLikes(post._id)} >Like</button>
			...
			*/
			...
			return <div key={index+1} className="posts_box">
					...
					post No:{index+1} <h4>Author:{post.authorName} {likeButton}{counting}</h4>	
