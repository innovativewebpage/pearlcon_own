...
function* addLikes(data)
{
	...
	//console.log('add_new_likes==',add_new_likes.data.data)
	
	 try {
	const add_new_likes = yield call(addLikesData);
		yield put({ type: ADD_LIKES_SUCCESS, payload: add_new_likes.data.data });
 } catch (error) {
...
const likes_api  = yield call(getLikesDataApi);
  ...
  yield put({ type: GET_ALLLIKES_SUCCESS, payload: likes_api.data.data });
 } catch (error) {
